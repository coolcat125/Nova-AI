...
class MetricBar(QWidget):
    ...
    def paintEvent(self, _):
        ...
        p.drawText(QRectF(8, 5, 50, 16), Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, self._label)
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self.document().setDefaultStyleSheet("p, div, li { line-height: 160%; }")
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure Nova before first boot.", 9, color=C.PRI_DIM))
        layout.addWidget(_lbl("One-time setup  --  saved to disk. Not per-session.", 8, color=C.TEXT_DIM))
        layout.addSpacing(6)
        ...
        self._remember_cb = QCheckBox("Remember settings (skip this screen next time)")
        self._remember_cb.setChecked(True)
        self._remember_cb.setFont(QFont("Courier New", 8))
        self._remember_cb.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent;")
    ...
    def _submit(self):
        ...
        if not key:
            ...
            return
        self.done.emit(key, self._sel_os, self._remember_cb.isChecked())
...
class MainWindow(QMainWindow):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        for txt, col, tip in [
            ("AI CORE\nACTIVE",     C.GREEN,   "Neural core initialized and running"),
            ("SEC\nCLEARED",        C.PRI,     "Security verification passed"),
            ("PROTOCOL\nI",         C.PRI,     "Primary agent protocol engaged"),
        ]:
            lbl = QLabel(txt)
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input.setStyleSheet(f"""
            QLineEdit {{
                ...
                border: 2px solid {C.BORDER_B}; border-radius: 4px; padding: 6px 10px;
            }}
    ...
    def _on_setup_done(self, key: str, os_name: str, remember: bool):

...
from PyQt6.QtWidgets import (
    ...
    QHBoxLayout, QLabel, QLineEdit, QListWidget, QListWidgetItem, QMainWindow, QMenu,
     QMessageBox, QPushButton, QScrollArea, QSizePolicy, QSplitter, QTextEdit, QDialog,
...
from config.paths import get_bundle_dir, get_data_dir

BASE_DIR    = get_bundle_dir()
CONFIG_DIR  = get_data_dir()
API_FILE    = CONFIG_DIR / ".env"
_THEME_FILE = CONFIG_DIR / "memory" / ".theme"
...
_DARK = {
    "BG": "#080010", "PANEL": "#0d0218", "PANEL2": "#12041e",
    "BORDER": "#1e0a34", "BORDER_B": "#3a1060", "BORDER_A": "#2e0d50",
    "PRI": "#c084fc", "PRI_DIM": "#7a3fa0", "PRI_GHO": "#1a0433",
    "ACC": "#ff3399", "ACC2": "#ff66cc",
    "GREEN": "#00ff88", "GREEN_D": "#00aa55",
    "RED": "#ff3355", "WARN": "#ffaa00",
    "MUTED_C": "#ff6699",
    "TEXT": "#d8b4fe", "TEXT_DIM": "#7a4a9a", "TEXT_MED": "#b07ad0",
    "WHITE": "#f0d8ff", "DARK": "#0a0014", "BAR_BG": "#150424",
}

_LIGHT = {
    "BG": "#f3eeff", "PANEL": "#e8deff", "PANEL2": "#ddd0ff",
    "BORDER": "#c8b5e8", "BORDER_B": "#a888d6", "BORDER_A": "#b89ae0",
    "PRI": "#7c3aed", "PRI_DIM": "#a070d0", "PRI_GHO": "#eee4ff",
    "ACC": "#cc0066", "ACC2": "#b82e8a",
    "GREEN": "#008844", "GREEN_D": "#00aa55",
    "RED": "#cc2244", "WARN": "#b87700",
    "MUTED_C": "#b83868",
    "TEXT": "#3a1060", "TEXT_DIM": "#8866aa", "TEXT_MED": "#663399",
    "WHITE": "#1a0033", "DARK": "#e0d2f8", "BAR_BG": "#d0bef0",
}
...
class HudCanvas(QWidget):
    def __init__(self, face_path: str = "", parent=None):
        ...
        self._face_px: Optional[QPixmap] = None
...
class LogWidget(QTextEdit):
    ...
    def _enqueue(self, text: str):
        ...
        if not self._loading and text.startswith("You:"):
            self._messages.append({"role": "user", "content": text[4:].strip()})
        elif not self._loading and text.startswith("Site:"):
            self._messages.append({"role": "user", "content": text[5:].strip()})
    ...
    def _finish_nova(self):
        if self._nova_buf and not self._loading:
            self._messages.append({"role": "assistant", "content": self._nova_buf})
        self._nova_has_prefix = False
    ...
    def load_messages(self, messages: list[dict]):
        ...
        for m in messages:
            ...
            if content:
                ...
                color = C.WHITE if role == "user" else C.PRI
                self._append_formatted(f"{prefix}{content}\n", color)
        self._messages = list(messages)
        self._loading = False

    def _append_formatted(self, text: str, color: str):
        cur = self.textCursor()
        cur.movePosition(cur.MoveOperation.End)
        fmt = cur.charFormat()
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("  Welcome to Nova", 22, True, C.WHITE))
        layout.addWidget(_lbl("One-time setup to get you started.", 14, color=C.PRI_DIM))
        layout.addSpacing(6)
        ...
        self._key_input.setFont(QFont("Courier New", 14, QFont.Weight.Bold))
        ...
        self._url_lbl = _lbl("BASE URL", 13, color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft)
        self._url_lbl.hide()
        layout.addWidget(self._url_lbl)
        self._url_input = QLineEdit()
        self._url_input.setPlaceholderText("http://localhost:11434/v1")
        self._url_input.setFont(QFont("Courier New", 14, QFont.Weight.Bold))
        ...
        layout.addWidget(_lbl("MICROPHONE TEST", 13, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        mic_row = QHBoxLayout(); mic_row.setSpacing(6)
        self._mic_test_btn = QPushButton("> Test Mic")
        self._mic_test_btn.setFont(QFont("Courier New", 14, QFont.Weight.Bold))
        self._mic_test_btn.setFixedSize(160, 36)
        ...
        self._mic_status.setFont(QFont("Courier New", 14, QFont.Weight.Bold))
    ...
    def _on_provider_change(self, idx: int):
        ...
        if prov == "gemini":
            ...
            self._key_toggle.show()
            self._key_link.setText(
                '<a href="https://platform.openai.com" style="color: #ff80d5; text-decoration: underline;">Get your OpenAI key <span style="font-size:16px;">&rarr;</span></a>'
            )
            self._key_link.setOpenExternalLinks(True)
            self._key_link.show()
    ...
    def _draw_checkmark(self, checked: bool):
        ...
        if checked:
            p = QPainter(pix)
            p.setRenderHint(QPainter.RenderHint.Antialiasing)
            pen = QPen(QColor(C.PRI), 3)
            pen.setCapStyle(Qt.PenCapStyle.RoundCap)
            p.setPen(pen)
            p.drawLine(5, 12, 10, 17)
            p.drawLine(10, 17, 19, 6)
            p.end()
    ...
    def apply_scale(self, s: float):
        self.layout().setContentsMargins(int(40 * s), int(32 * s), int(40 * s), int(32 * s))
        self.layout().setSpacing(int(6 * s))
...
class ShortcutOverlay(QWidget):
    def __init__(self, parent, win):
        ...
        binds = [
            ...
            ("Ctrl+L", "Focus input"),
            ("Ctrl+U", "Open file"),
            ("Ctrl+B", "Toggle Chat On Site"),
            ("Ctrl+K", "Show shortcuts"),
            ("Ctrl+Q", "Quit"),
            ("Ctrl+R", "Reload Gemini"),
            ("Ctrl+Tab", "Toggle tool confirm"),
            ("F4", "Mute microphone"),
            ("F5", "Deafen"),
            ("F6", "Toggle theme"),
            ("F11", "Fullscreen"),
        ...
        for key, desc in binds:
            ...
            kl = QLabel(key)
            kl.setFont(QFont("Courier New", 11, QFont.Weight.Bold))
            kl.setStyleSheet(f"color: {C.ACC2}; background: transparent;")
            kl.setFixedWidth(80)
            kl.setAlignment(Qt.AlignmentFlag.AlignRight)
            row.addWidget(kl)
            dl = QLabel(desc)
            dl.setFont(QFont("Courier New", 11, QFont.Weight.Bold))
...
class ThemeSwitch(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._knob_norm = 1.0
        self._press_x = 0.0
        self._slide_tmr = QTimer(self)
        self._slide_tmr.timeout.connect(self._slide_step)
        self._slide_progress = 0.0
        self._slide_from = 0.0
        self._slide_to_val = 0.0
        self._slide_theme = ""
        self.setMouseTracking(True)

    def set_theme(self, name: str):
        self._theme = name
        self._knob_norm = 1.0 if name == "dark" else 0.0
        self.update()

    def slide_to(self, name: str):
        if name == self._theme:
            return
        self._slide_theme = name
        self._slide_from = self._knob_norm
        self._slide_to_val = 1.0 if name == "dark" else 0.0
        self._slide_progress = 0.0
        self._theme = name
        self._slide_tmr.start(16)

    def _slide_step(self):
        self._slide_progress += 0.12
        if self._slide_progress >= 1.0:
            self._slide_tmr.stop()
            self._knob_norm = self._slide_to_val
            self.update()
            self.toggled.emit(self._slide_theme)
        else:
            t = 1 - (1 - self._slide_progress) ** 3
            self._knob_norm = self._slide_from + (self._slide_to_val - self._slide_from) * t
            self.update()
    ...
    def _knob_x(self):
        ...
        w = self.width()
        span = w - h - 2
        return 1 + int(self._knob_norm * span)
    ...
    def paintEvent(self, event):
        ...
        w = self.width()
        r = h / 2
        active = self._theme == "light"

        p.setPen(QPen(qcol(C.BORDER_B if active else C.BORDER), 1))
        bg = qcol(C.PRI_GHO if active else C.PANEL2)
        ...
        p.drawEllipse(kx + 3, 3, h - 6, h - 6)

        p.setFont(QFont("Segoe UI", 10))
        icon_color = qcol(C.WHITE if active else C.TEXT_DIM)
        p.setPen(QPen(icon_color, 1))
        p.drawText(QRectF(2, 0, h, h), Qt.AlignmentFlag.AlignCenter, "\u2600")
        icon_color = qcol(C.WHITE if not active else C.TEXT_DIM)
        p.setPen(QPen(icon_color, 1))
        p.drawText(QRectF(w - h - 2, 0, h, h), Qt.AlignmentFlag.AlignCenter, "\u263E")
    ...
    def mouseMoveEvent(self, event):
        if self._dragging:
            ...
            w = self.width()
            span = w - h - 2
            self._knob_norm = max(0.0, min(1.0, (event.position().x() - 1) / span))
            self.update()

    def mouseReleaseEvent(self, event):
        if self._dragging:
            self._dragging = False
            dx = abs(event.position().x() - self._press_x)
            if dx < 5:
                snap = "light" if self._theme == "dark" else "dark"
            else:
                snap = "dark" if self._knob_norm > 0.5 else "light"
            if snap != self._theme:
                self.slide_to(snap)
            else:
                self._knob_norm = 1.0 if self._theme == "dark" else 0.0
                self.update()


class ToolConfirmSwitch(QWidget):
    ...
    def set_on(self, on: bool):
        self._on = on
        self._knob_norm = 1.0 if on else 0.0
        self.update()

    def slide_to(self, on: bool):
        if on == self._on:
            return
        self._slide_from = self._knob_norm
        self._slide_to_val = 1.0 if on else 0.0
        self._slide_progress = 0.0
        self._on = on
        self._slide_tmr.start(16)
    ...
    def paintEvent(self, event):
        ...
        active = self._on
        p.setPen(QPen(qcol(C.BORDER_B if active else C.BORDER), 1))
        bg = qcol(C.ACC if active else C.PANEL2)
        p.setBrush(QBrush(bg))
        p.drawRoundedRect(1, 1, w - 2, h - 2, r, r)
    ...
    def mouseReleaseEvent(self, event):
        if self._dragging:
            ...
            if dx < 5:
                ...
                snap = self._knob_norm > 0.5
            if snap != self._on:
                self.slide_to(snap)
            else:
                self._knob_norm = 1.0 if self._on else 0.0
                self.update()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str = ""):
        ...
        self._orb_opacity: Optional[QGraphicsOpacityEffect] = None
        self._orb_anim: Optional[QPropertyAnimation] = None
        self._font_scale = 1.0
        self._base_font_sizes: dict[int, int] = {}
        self._base_font_bold: dict[int, bool] = {}
        ...
        self._bridge_code    = ""
        self._bridge: "Optional[TextBridge]" = None
        self._bridge_overlay: Optional[SiteLinkOverlay] = None
        ...
        self._theme_switch = ThemeSwitch(self.hud)
        self._theme_switch.toggled.connect(self._on_theme_toggled)
        self._tool_confirm = True
        self._tool_confirm_switch = ToolConfirmSwitch(self.hud)
        self._tool_confirm_switch.set_on(True)
        self._tool_confirm_switch.toggled.connect(self._on_tool_confirm_toggled)
        QTimer.singleShot(0, self._pos_theme_switch)
        ...
        self._nova_end_sig.connect(self._on_turn_complete)
        self._state_sig.connect(self._on_state_signal)
        self._title_sig.connect(self._on_title_suggested)
        ...
        self._theme_sig.connect(self._on_theme_toggled)

        sc_mute = QShortcut(QKeySequence("F4"), self)
        sc_mute.activated.connect(self._toggle_mute)
        sc_deaf = QShortcut(QKeySequence("F5"), self)
        sc_deaf.activated.connect(self._toggle_deafen)
        sc_theme = QShortcut(QKeySequence("F6"), self)
        sc_theme.activated.connect(self._toggle_theme)
        sc_full = QShortcut(QKeySequence("F11"), self)
        sc_full.activated.connect(self._toggle_fullscreen)
        sc_file = QShortcut(QKeySequence("Ctrl+U"), self)
        sc_file.activated.connect(self._open_file)
        ...
        sc_bridge = QShortcut(QKeySequence("Ctrl+B"), self)
        sc_bridge.activated.connect(self._toggle_bridge)
        sc_new = QShortcut(QKeySequence("Ctrl+N"), self)
        sc_new.activated.connect(self._new_session)
        sc_help = QShortcut(QKeySequence("Ctrl+K"), self)
        sc_help.activated.connect(self._show_shortcuts)
        sc_confirm = QShortcut(QKeySequence("Ctrl+Tab"), self)
        sc_confirm.activated.connect(self._toggle_tool_confirm)
        sc_reload = QShortcut(QKeySequence("Ctrl+R"), self)
        sc_reload.activated.connect(self._reload_gemini)

        self._load_theme()
        self._log.append_log(f"SYS: Tool confirmation {'ON' if self._tool_confirm else 'OFF'}")
    ...
    def _toggle_theme(self):
        nxt = "light" if C._current == "dark" else "dark"
        self._theme_switch.slide_to(nxt)

    def _toggle_tool_confirm(self):
        nxt = not self._tool_confirm
        self._tool_confirm = nxt
        self._tool_confirm_switch.slide_to(nxt)

    def _reload_gemini(self):
        if self._nova_client:
            self._nova_client.reset()

    def _load_theme(self):
        try:
            saved = _THEME_FILE.read_text(encoding="utf-8").strip()
            if saved in ("dark", "light"):
                self._on_theme_toggled(saved)
                self._theme_switch.set_theme(saved)
    ...
    def _on_theme_toggled(self, name: str):
        old = _DARK if name == "light" else _LIGHT
        new = _LIGHT if name == "light" else _DARK
        set_theme(name)
        self._retheme(old, new)
        self._save_theme(name)
        self._log.append_log(f"SYS: {'Dark' if name == 'dark' else 'Light'} theme")

    def _retheme(self, old: dict, new: dict):
        cw = self.centralWidget()
        for w in [cw] + cw.findChildren(QWidget):
            ss = w.styleSheet()
            if ss:
                for k in old:
                    ss = ss.replace(old[k], new[k])
                w.setStyleSheet(ss)
        self._style_mic_btn()
        self._style_deaf_btn()
        self.hud.update()
        self._theme_switch.update()
        self._tool_confirm_switch.update()
        for bar in [self._bar_cpu, self._bar_mem, self._bar_net, self._bar_gpu, self._bar_tmp]:
            bar.update()
        if hasattr(self, "_drop_zone") and hasattr(self._drop_zone, "_canvas"):
            self._drop_zone._canvas.update()
        self._session_list.viewport().update()
        self._apply_font_weight()

    def _apply_font_weight(self):
        bold = C._current == "light"
        cw = self.centralWidget()
        for w in [cw] + cw.findChildren(QWidget):
            if isinstance(w, (QLabel, QPushButton, QLineEdit, QTextEdit, QListWidget, QComboBox)):
                wid = id(w)
                if wid not in self._base_font_bold:
                    self._base_font_bold[wid] = w.font().bold()
                target = bold or self._base_font_bold[wid]
                f = w.font()
                if f.bold() != target:
                    f.setBold(target)
                    w.setFont(f)

    def _on_tool_confirm_toggled(self, on: bool):
        self._tool_confirm = on
        self._log.append_log(f"SYS: Tool confirmation {'ON' if on else 'OFF'}")

    _TOOL_FRIENDLY = {
        "screen_process": "view your screen",
        "open_app": "open an app",
        "weather_report": "check weather",
        "browser_control": "control browser",
        "file_controller": "manage files",
        "send_message": "send a message",
        "reminder": "set a reminder",
        "youtube_video": "play YouTube",
        "computer_settings": "change system settings",
        "desktop_control": "control desktop",
        "code_helper": "help with code",
        "dev_agent": "run dev agent",
        "agent_task": "queue a task",
        "web_search": "search the web",
        "file_processor": "process a file",
        "computer_control": "control computer (mouse/keyboard)",
        "game_updater": "update games",
        "flight_finder": "find flights",
        "shutdown_nova": "shut down Nova",
    }

    async def _confirm_tool_dialog(self, name: str, args: dict) -> bool:
        import asyncio
        from PyQt6.QtWidgets import QApplication

        self._confirm_result = None
        friendly = self._TOOL_FRIENDLY.get(name, name)
        text = f"Allow Nova to <b>{friendly}</b>?"
    ...
    def _pos_theme_switch(self):
        gap = 6
        x = self.hud.width() - self._theme_switch.width() - 8
        self._theme_switch.move(x, 8)
        cx = x - self._tool_confirm_switch.width() - gap
        self._tool_confirm_switch.move(cx, 8)
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        sess_hdr.setFont(QFont("Courier New", 9, QFont.Weight.Bold))
        sess_hdr.setStyleSheet(f"color: {C.ACC2}; background: transparent; "
                               f"border-bottom: 1px solid {C.BORDER}; padding-bottom: 2px; margin-top: 4px;")
        lay.addWidget(sess_hdr)

        self._session_list = QListWidget()
        self._session_list.setMinimumHeight(80)
        self._session_list.setFont(QFont("Courier New", 11))
        ...
        lay.addWidget(self._session_list)

        sess_btn_row = QHBoxLayout()
        sess_btn_row.setSpacing(4)
        self._rename_btn = QPushButton("RENAME")
        self._rename_btn.setFixedHeight(22)
        self._rename_btn.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        ...
        """)
        self._rename_btn.clicked.connect(self._rename_selected_session)
        sess_btn_row.addWidget(self._rename_btn)
        self._delete_btn = QPushButton("DELETE")
        self._delete_btn.setFixedHeight(22)
        self._delete_btn.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        ...
        self._delete_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ color: {C.RED}; border: 1px solid {C.BORDER_B}; }}
        """)
        self._delete_btn.clicked.connect(self._delete_selected_session)
        sess_btn_row.addWidget(self._delete_btn)
        lay.addLayout(sess_btn_row)
        ...
        kb_btn = QPushButton("KEYBOARD SHORTCUTS")
        kb_btn.setFixedHeight(24)
        kb_btn.setFont(QFont("Courier New", 9, QFont.Weight.Bold))
        ...
        kb_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ color: {C.TEXT}; border: 1px solid {C.BORDER_B}; }}
        """)
        kb_btn.clicked.connect(self._show_shortcuts)
        lay.addWidget(kb_btn)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._confirm_bar.setFixedHeight(44)
        self._confirm_bar.setStyleSheet(f"background: {C.PANEL2}; border: 1px solid {C.BORDER_A}; border-radius: 4px;")
        cb_lay = QHBoxLayout(self._confirm_bar)
        cb_lay.setContentsMargins(8, 2, 8, 2)
        self._confirm_label = QLabel()
        self._confirm_label.setStyleSheet(f"color: {C.WHITE}; font-size: 10pt; background: transparent;")
        cb_lay.addWidget(self._confirm_label, stretch=1)

        deny_btn = QPushButton("Deny")
        deny_btn.setFixedSize(60, 28)
        ...
        deny_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ background: {C.PANEL2}; border: 1px solid {C.RED}; }}
        """)
        deny_btn.clicked.connect(lambda: setattr(self, "_confirm_result", False))
        cb_lay.addWidget(deny_btn)

        allow_btn = QPushButton("Allow")
        allow_btn.setFixedSize(60, 28)
        allow_btn.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        ...
        allow_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ background: {C.PANEL2}; border: 1px solid {C.GREEN}; }}
        """)
        allow_btn.clicked.connect(lambda: setattr(self, "_confirm_result", True))
        cb_lay.addWidget(allow_btn)
        self._confirm_bar.hide()
        lay.addWidget(self._confirm_bar)
        ...
        new_btn.setFont(QFont("Courier New", 10, QFont.Weight.Bold))
    ...
    def _show_shortcuts(self):
        if hasattr(self, "_shortcut_overlay") and self._shortcut_overlay and self._shortcut_overlay.isVisible():
            self._shortcut_overlay.hide()
            return
        ov = ShortcutOverlay(self.centralWidget(), self)
        ov.center_on_parent()
        ov.show()
        ov._focus()
    ...
    def _refresh_session_list(self):
        ...
        cur_id = self._session_mgr.get_current_id()
    ...
    def _on_session_clicked(self, item):
        ...
        if not sid or sid == self._session_mgr.get_current_id():
            return
        self._save_current_session()
        msgs = self._session_mgr.load_messages(sid)
        self._session_mgr.set_current_id(sid)
        self._log.load_messages(msgs)
        self._log.append_log("SYS: Session loaded.")
        self._refresh_session_list()

    def _save_current_session(self):
        sid = self._session_mgr.get_current_id()
        if sid:
            msgs = self._log.get_messages()
            self._session_mgr.save_messages(sid, msgs)
    ...
    def _on_turn_complete(self):
        ...
        if msgs:
            ...
            self._refresh_session_list()
            if len(msgs) >= 2:
                threading.Thread(target=self._suggest_title, args=(msgs,), daemon=True).start()

    def _suggest_title(self, messages: list):
        sid = self._session_mgr.get_current_id()
        if not sid:
            return
        cur = next((s for s in self._session_mgr.list_sessions() if s["id"] == sid), None)
        if cur and cur["title"] not in ("New session",) and not cur["title"].startswith("Session "):
            return
        conv = "\n".join(
        ...
        try:
            ...
            if title and len(title) < 60:
                self._title_sig.emit(sid, title)
        except Exception as e:
            print(f"[Nova] Title gen error: {e}")

    def _on_title_suggested(self, sid: str, title: str):
        self._session_mgr.rename_session(sid, title)
        msgs = self._session_mgr.load_messages(sid)
        self._session_mgr.save_messages(sid, msgs, title=title)
        self._refresh_session_list()
        if sid == self._session_mgr.get_current_id():
            self._log.append_log(f"SYS: Session renamed to \"{title}\"")
    ...
    def _rename_selected_session(self):
        ...
        title, ok = QInputDialog.getText(self, "Rename Session", "Title:", text=item.text()[:35].strip())
        if ok and title:
            self._session_mgr.rename_session(sid, title.strip())
            self._refresh_session_list()
    ...
    def _delete_selected_session(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if self._session_mgr.get_current_id() is None:
                self._session_mgr.create_session()
            self._refresh_session_list()
            self._log.append_log("SYS: Session deleted.")
        ...
        if self._bridge_active:
            ...
            if self._bridge is None:
                ...
                self._bridge.set_callbacks(
                    on_message=lambda t: self._on_bridge_message(t),
    ...
    def _on_bridge_message(self, text: str):
        self._log_sig.emit(f"Site: {text}")
        self._nova_end_sig.emit()
    ...
    def _style_mic_btn(self):
        is_light = C._current == "light"
        if self._muted:
            self._mic_btn.setText("MUTED")
            m_bg = "#fce8f0" if is_light else "#140006"
            m_hv = "#fce0ec" if is_light else "#1e000a"
            self._mic_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {m_bg}; color: {C.MUTED_C};
                    border: 1px solid {C.MUTED_C}; border-radius: 4px;
                }}
                QPushButton:hover {{ background: {m_hv}; }}
            """)
        else:
            self._mic_btn.setText("MUTE")
            a_bg = "#e0f5ea" if is_light else "#00140a"
            a_hv = "#d0f0e0" if is_light else "#001f10"
            self._mic_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {a_bg}; color: {C.GREEN};
                    border: 1px solid {C.GREEN}; border-radius: 4px;
                }}
                QPushButton:hover {{ background: {a_hv}; }}
            """)

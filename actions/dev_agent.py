...
PROJECTS_DIR     = Path.home() / "Desktop" / "NovaProjects"
MAX_FIX_ATTEMPTS = 5
...
def _plan_project(description: str, language: str) -> dict:
    client = _get_client(MODEL_PLANNER)
...
def _write_file(
    ...
    client = _get_client(MODEL_WRITER)
...
def _install_dependencies(dependencies: list[str], project_dir: Path) -> str:
    ...
    for dep in dependencies:
        ...
        if result.returncode != 0:
            ...
            print(f"[DevAgent] [OK] Already installed: {pkg_name}")
...
def _build_project(
    ...
    proj_name    = project_name or plan.get("project_name", "nova_project")

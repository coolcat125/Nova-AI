...
def _capture_screen() -> tuple[bytes, str]:
    ...
    with mss.mss() as sct:
        ...
        for i in range(1, len(monitors)):
            ...
            frames.append(img)
            total_w += shot.size[0]
            max_h = max(max_h, shot.size[1])

        canvas = PIL.Image.new("RGB", (total_w, max_h))
        x = 0
        for img in frames:
        ...
        buf = io.BytesIO()
        canvas.save(buf, format="JPEG", quality=_JPEG_Q, optimize=False)
        return buf.getvalue(), "image/jpeg"
...
def _cv2_backend() -> int:
    ...
    if not _CV2:
        return 0
    os_name = _get_os()
    if os_name == "windows": return cv2.CAP_DSHOW
    if os_name == "darwin":  return cv2.CAP_AVFOUNDATION
    return cv2.CAP_ANY
...
def _detect_camera_index() -> int:
    ...
    print("[Vision] [search] Auto-detecting camera...")
    for idx in range(6):
            ...
            try:
                ...
                async with client.aio.live.connect(
                    ...
                    try:
                        ...
                        print(f"[Vision] [WARN]  Session error: {exc}")
            finally:
                self._session = None
    ...
    async def _send_loop(self) -> None:
        while True:
            ...
            if not self._session:
                print("[Vision] [WARN]  No session  --  dropping image")
                continue
            try:
                ...
                print(f"[Vision] [WARN]  Send error: {e}")
...
def screen_process(
    ...
    if not user_text:
        print("[Vision] [WARN]  No question provided  --  aborting")
        return False
    ...
    try:
        if angle == "camera":
            ...
            print(f"[Vision] [screen]  Screen: {len(image_bytes):,} bytes")
    except Exception as e:

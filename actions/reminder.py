...
try:
    from plyer import notification
        notification.notify(title="Nova Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("Nova Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
"""
    ...
    script_body = f"""# Auto-generated by Nova reminder  --  do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>Nova Reminder</Description></RegistrationInfo>\n'
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.nova.reminder.{task_name}"
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        print(f"[Reminder] [WARN] systemd-run failed: {result.stderr.strip()}, trying 'at'")

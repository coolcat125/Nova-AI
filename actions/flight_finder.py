...
def _parse_date(raw: str) -> str:
    ...
    try:
        ...
        print(f"[FlightFinder] [WARN] Gemini date parse failed: {e}")
    ...
    # Last resort: today
    print(f"[FlightFinder] [WARN] Could not parse date '{raw}'  --  using today.")
...
def _search_flights_browser(
    ...
    print(f"[FlightFinder] [web] Opening: {url}")
...
def _parse_flights_with_gemini(
    ...
    try:
        ...
        response = client.models.generate_content(
            model="gemini-2.5-flash-native-audio-latest",
            contents=prompt,
            config={"system_instruction": "You are a flight data extraction expert. Extract flight information from raw webpage text. Return ONLY valid JSON  --  no markdown, no explanation."}
        )
        text     = re.sub(r"```(?:json)?", "", response.text).strip().rstrip("`").strip()
        flights  = json.loads(text)
        return flights if isinstance(flights, list) else []
    except Exception as e:
        print(f"[FlightFinder] [WARN] Gemini parse failed: {e}")
        return []
...
def _save_to_desktop(content: str, origin: str, destination: str) -> str:
    ...
    try:
        ...
        print(f"[FlightFinder] [WARN] Could not open text editor: {e}")
...
def flight_finder(parameters: dict, player=None, speak=None) -> str:
    ...
    print(
        f"[FlightFinder] [play] {origin}  ->  {destination} | {date}"

...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".nova_profiles" / browser
...
def _resolve_browser(name: str) -> Optional[dict]:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".nova_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Nova profile")
                nova_fb = str(Path.home() / ".nova_profiles" / "firefox_nova")
                Path(nova_fb).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(nova_fb, **kwargs)
        ...
        nova_profile = str(Path.home() / ".nova_profiles" / self.browser_name)
        Path(nova_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Nova profile: {nova_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(nova_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] [OK] Launched [{label}] with Nova profile")

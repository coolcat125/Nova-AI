...
_FP_MODEL = "gemini-2.5-flash-native-audio-latest"
...
def _process_image(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("describe", "ocr", "analyze", "read", "extract_text"):
        try:
            ...
            response = client.models.generate_content(model=_FP_MODEL, contents=[prompt, img])
...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("summarize", "extract_text", "translate_hint", "analyze", "reformat"):
        ...
        try:
            client = _gemini_client()
            from actions.quota_tracker import increment as increment_quota; increment_quota()
            response = client.models.generate_content(model=_FP_MODEL, contents=prompt_map.get(action, f"Analyze:\n\n{text}"))
            result   = response.text.strip()
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    try:
        client = _gemini_client()
        from actions.quota_tracker import increment as increment_quota; increment_quota()
        response = client.models.generate_content(model=_FP_MODEL, contents=prompt_map[action])
        result   = response.text.strip()
        if len(result) > 600 and params.get("save", True):
            out = _output_path(path, action, ".txt")
            out.write_text(result, encoding="utf-8")
...
def _process_data(path: Path, file_type: str, action: str,
    ...
    if action == "analyze":
        ...
        try:
            client = _gemini_client()
            from actions.quota_tracker import increment as increment_quota; increment_quota()
            response = client.models.generate_content(model=_FP_MODEL, contents=prompt)
    ...
    try:
        client = _gemini_client()
        from actions.quota_tracker import increment as increment_quota; increment_quota()
        response = client.models.generate_content(model=_FP_MODEL, contents=

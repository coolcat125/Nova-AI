...
def _call_gemini(client, model: str, contents: str, max_retries: int = 3):
    ...
    for attempt in range(max_retries + 1):
        try:
            from actions.quota_tracker import increment as _inc; _inc()
            resp = client.models.generate_content(model=model, contents=contents)
            return resp
        except Exception as e:
            err_str = str(e)
            is_retriable = any(
                str(code) in err_str or f"HTTP {code}" in err_str
                for code in _RETRIABLE_CODES
            ) or "RESOURCE_EXHAUSTED" in err_str.upper()
            if not is_retriable or attempt >= max_retries:
                raise
            delay = (2 ** attempt) + random.random()
            print(f"[Gemini] Retry {attempt+1}/{max_retries} after {delay:.1f}s: {e}")
            time.sleep(delay)
...
def _take_screenshot() -> Optional[Path]:
    try:
        ...
        print(f"[Code] [WARN] Screenshot failed: {e}")
        return None
...
Code:"""
    ...
    response = _call_gemini(client, GEMINI_MODEL, prompt)
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    if file_path:
        ...
        if err:
            print(f"[Code] [WARN] Could not read file: {err}")
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        print(f"[Code] [OK] Screen analysis complete")
